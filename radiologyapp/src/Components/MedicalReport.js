...
const MedicalReport = ({ results , xray}) => {
...
//     const [mostLikelyDiagnosis, probability] = findings[0];

//     let impression = '';
//     if (probability >= 0.9) {
//       impression = `The ${mostLikelyDiagnosis} is the most likely diagnosis with a high degree of confidence, based on the chest x-ray findings. The radiographic features include ${getRadiographicFeatures(mostLikelyDiagnosis)}.`;
//     } else if (probability >= 0.7) {
//       impression = `The chest x-ray shows findings suggestive of ${mostLikelyDiagnosis} with a moderate degree of confidence. The radiographic features include ${getRadiographicFeatures(mostLikelyDiagnosis)}.`;
//     } else if (probability >= 0.5) {
//       impression = `The chest x-ray shows findings suggestive of ${mostLikelyDiagnosis}. The radiographic features include ${getRadiographicFeatures(mostLikelyDiagnosis)}. Further imaging or clinical correlation is recommended to confirm the diagnosis.`;
//     } else {
//       impression = `The chest x-ray is within normal limits, with no radiographic evidence of any significant abnormalities or pathological findings.`;
//     }
...
//     let report = '';
//     findings.forEach(([diagnosis, probability]) => {
//       const probabilityPercent = Math.round(probability * 100);
//       report += `${diagnosis}: ${probabilityPercent}% certainty\n`;
//     });

//     return report;
//   };

//   // Helper function to get radiographic features based on the diagnosis
//   const getRadiographicFeatures = (diagnosis) => {
//     switch (diagnosis) {
//       case 'pneumonia':
//         return 'patchy or diffuse areas of consolidation, air bronchograms, and/or pleural effusions.';
//       case 'pneumothorax':
//         return 'absent lung markings, visceral pleural line, or a deep sulcus sign.';
//       case 'atelectasis':
//         return 'volume loss, displacement of fissures, elevation of the diaphragm, and/or opacification of the affected lung segment.';
//       case 'lung mass':
//         return 'a focal opacity with poorly defined margins, irregular shape, and/or spiculated borders.';
//       case 'pleural effusion':
//         return 'blunting of the costophrenic angle, meniscus sign, and/or obscuring of the diaphragm.';
//       default:
//         return 'no specific radiographic features to suggest any significant pathology.';
//     }
//   };
  
//   // Generate the report text
//   const impression = generateImpression(results);
//   const findings = generateFindings(results);
//   const date = formatDate(new Date());
//   const reportText = `IMPRESSION: ${impression}\n\nFINDINGS: ${findings}\n\nDATE OF REPORT: ${date}\n`;
